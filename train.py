...
RNN = flags.DEFINE_bool("rnn", False, "")
RNN = flags.DEFINE_integer("num_freqs", 8, "")
...
def sequence_loss(batch: dataset.Batch) -> jnp.ndarray:
    ...
    if flags.FLAGS.rnn:
        ...
        one_hots = jax.nn.one_hot(batch["input"], num_classes=dataset.NUM_CHARS)
        widths = 2 ** np.arange(flags.FLAGS.num_freqs)
        cumsum = np.cumsum(widths)
        d = cumsum[-1]
        embed = hk.Linear(d)(one_hots)
        embed = jax.nn.relu(embed)
        # embed = np.ones_like(embed)
        # for i in range(len(embed)):
        #     embed[i] = i
        t, b, d = embed.shape
        patch_ids = np.ones((t, 1, d))
        start = 0
        for i, stop in enumerate(cumsum):
            patch_ids[:, :, start:stop] = i
            start = stop
        patch_ids = jax.device_put(patch_ids)

        patched = [
            sum(
                jnp.roll(p, shift=j * width, axis=-1)
                for j, p in enumerate(embed * (patch_ids == i))
            )
        ...
        #         for j, p in enumerate(embed * patch_ids == i)
        #     )
        #     patched.append(rolled)

        concatenate = jnp.concatenate(patched, axis=-1)
        hidden = hk.Linear(HIDDEN_SIZE.value)(concatenate)
        hidden = jax.nn.relu(hidden)
        logits = hk.Linear(dataset.NUM_CHARS)(hidden)
